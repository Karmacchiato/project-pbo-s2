...
<head>
    ...
    <script>
        function updateHari() {
            var tanggalInput = document.getElementById("tanggal_bimbingan").value;
            var date = new Date(tanggalInput);
            var hari = date.toLocaleDateString('id-ID', { weekday: 'long' });

            document.getElementById("hari_bimbingan").value = hari;
        }
    </script>
                ...
                <div class="table-responsive">
                    <form action="" method="POST" class="form-item">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="tanggal_bimbingan" id="tanggal_bimbingan" class="form-control col-md-9" onchange="updateHari()">
                        </div>

                        <!-- Tambahkan input untuk memilih hari bimbingan -->
                        <div class="form-group">
                            <label for="hari_bimbingan">Hari Bimbingan</label>
                            <input type="text" name="hari_bimbingan" id="hari_bimbingan" class="form-control col-md-9" readonly>
                        </div>
                        ...
                        <button type="reset" class="btn custom-btn-color-warning">Hapus</button>
                    </form>
                </div>
            </div>
        </div>
...
if(isset($_POST['submit'])){
    ...
    $hari_bimbingan = $_POST['hari_bimbingan'];
    $ruangan = $_POST['ruangan'];

    $sql = "UPDATE tl_bimbingan SET tanggal_bimbingan='$tanggal_bimbingan', hari_bimbingan='$hari_bimbingan', ruangan='$ruangan' WHERE id_user='$id_user'";
